...
# Source: https://automaticaddison.com/value-iteration-vs-q-learning-algorithm-in-python-step-by-step/#valueiteration
 
# Define constants
ALGORITHM_NAME = "Value_Iteration"
# FILENAME = "data/tiny-track.txt"
...
def act(old_y, old_x, old_vy, old_vx, accel, environment, deterministic=(False),bad_crash = False):
    ...
    temp_y, temp_x = get_new_position((old_y,old_x), (new_vy, new_vx),(environment))
...
def value_iteration(environment, bad_crash = False, reward = (0.0),  no_training_iter = NO_TRAINING_ITERATIONS):
    global internal_timer
    ...
    for t in range(no_training_iter):
                ...
                for vy in vel_range:
                    for vx in vel_range:
                        ...
                        for ai, a in enumerate(actions):
                            ...
                            new_y, new_x, new_vy, new_vx = act(
                                y,x,vy,vx,(0,0),environment,deterministic = (True), bad_crash = bad_crash)
 
                            # V(s'): value of the new state when taking action
                            # (0,0) from state s. This is the value if for some
                            # reason the race car attemps to accelerate but fails
                            value_of_new_state_if_action_fails = values_prev[new_y][new_x][new_vy][new_vx]
        ...
        print('internal timer for restart position:\t{:.3f}s'.format(internal_timer))
        internal_timer = 0
...
def do_time_trial(environment, policy, bad_crash = False, animate = True, max_steps = MAX_STEPS):
...
def main():
    ...
    else: print("Bad Crash Scenario: " + option_2)
    print("Average Number of Steps the Race Car Needs to Take Before " + "Finding the Finish Line: " + str(total_steps/races) + " steps\n")
    print("\nThe race car is training. Please wait...")
    ...
    outfile_ts.write("Average Number of Steps the Race Car Took " +
            "Before Finding the Finish Line: " + str(total_steps/races) + " steps\n")
