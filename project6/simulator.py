...
"""
    Class representing the Track object, loaded from the track files
        - provides methods for moving, acceleration, crash restarting, boundary checking, pretty printing
...
class TrackSimulator():
    ...
    # helper to return number of rows in track grid
    def nrows(self):
        return self.track.shape[0]

    # helper to return number of columns in track grid
    def ncols(self):
        return self.track.shape[1]

    # helepr to pretty print race track, with row/column number headers
    # will add a green X printout for the current position
...
####################################################################################
    ...
    # get random start position within the track
    def start(self):
    ...
    """
        ...
        arguments:
            - old_position: current position of car on track
            - position: potential new position for car to move
    ...
    """
        two handling strategies
            ...
            uses internal crash_restart flag to determine which strategy
        
        arguments:
            - crash_site: position of where the car crashed during training/trial
    ...
    """
        runs time trial on track given a policy
            - number of max moves to take were capped at 300 or 500 to prevent excessive runtime

        arguments:
            - policy: dictionary state-action combos, generated by learner algorithm
